// ============================================================
// 훈련지변경 데이터 수집 스크립트 (목록 + 상세, 중단 재개 지원)
// 사용법: npm run fetch-training-change
...
// --- 타입 ---
interface TrainingChangeListItem {
  seqId: string;
  title: string;
  regDate: string;
}

interface TrainingChangeDetail {
  seqId: string;
  title: string;
  regDate: string;
...
const PROGRESS_PATH = path.join(DATA_DIR, "training-change-progress.json");
...
// --- 목록 페이지 파싱 ---
function parseListPage(html: string): { items: TrainingChangeListItem[]; lastPage: number } {
  const items: TrainingChangeListItem[] = [];
...
// --- 상세 페이지 파싱 ---
function parseDetailPage(html: string, listItem: TrainingChangeListItem): TrainingChangeDetail {
  let bodyText = "";

  // 패턴 1: view/content 영역
...
// --- Progress ---
function loadProgress(): { processedIds: Set<string>; records: TrainingChangeDetail[] } {
...
function saveProgress(processedIds: Set<string>, records: TrainingChangeDetail[]): void {
...
// --- 메인 ---
async function main() {
  console.log("훈련지변경 데이터 수집 시작...");

  // Phase 1: 전체 목록 수집
  console.log("\n--- Phase 1: 목록 수집 ---");
  const allItems: TrainingChangeListItem[] = [];
  let page = 1;
  let lastPage = 1;
  ...
  while (page <= lastPage) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES && !success; attempt++) {
      try {
        const url = "https://www.kcycle.or.kr/racer/state/trainingsitechange";
        const res = page === 1
          ? await fetch(url)
          : await fetch(url, {
              method: "POST",
  ...
  for (let i = 0; i < todoItems.length; i++) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES && !success; attempt++) {
      try {
        const url = `https://www.kcycle.or.kr/racer/state/trainingsitechange/${item.seqId}`;
  ...
  // 저장
  const outPath = path.join(DATA_DIR, "training-change-data.json");
