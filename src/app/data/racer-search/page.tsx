...
export default function RacerSearchPage() {
  ...
  const groupedByYear = new Map<number, RacerResult[]>();
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <p className="text-sm text-muted-foreground">
            총 {results.length}건 검색됨 {results.length >= 200 && "(최대 200건)"}
          </p>

          {sortedYears.map((year) => (
            <div key={year}>
              <h2 className="mb-3 text-lg font-semibold">{year}년</h2>
              <div className="space-y-3">
                {groupedByYear.get(year)!.map((racer) => {
                  const uid = `${racer.racerId}-${racer.year}`;
                        ...
                        <CardContent className="space-y-4 pt-0">
                          ...
                          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-3">
                            <div>
                              ...
                              <span className="font-medium">{racer.training}</span>
                            </div>
                            <div>
